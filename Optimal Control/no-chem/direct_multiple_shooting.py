					...
					for use_U in use_U_list:
						...
						for lambda_val in lambda_list:
							...
							if use_U:
								...
								ax.plot(time_solution, U_solution, 'c-')
								ax.set(xlabel = r'Time (h)', ylabel = r'Control integral $U$')
								plt.savefig("Figure_" + str(fig_number) + "_control_integral.pdf", bbox_inches = 'tight')
								plt.close()

								output_array = np.column_stack((time_solution.flatten(), U_solution.flatten()))
								np.savetxt("data_control_integral.csv", output_array, delimiter=',', fmt='%.8f',
										   header="time, U", comments='')

							fig, ax = plt.subplots(1, 1, dpi = 300)
							ax.plot(time_solution, np.append(np.nan, u_solution), 'm-')
							ax.set(xlabel = r'Time (h)', ylabel = r'Control $u$')
							plt.savefig("Figure_" + str(fig_number) + "_control.pdf", bbox_inches = 'tight')
							plt.close()

							output_array = np.column_stack((time_solution.flatten(), np.append(np.nan, u_solution)))
							np.savetxt("data_control.csv", output_array, delimiter=',', fmt='%.8f',
									   header="time, u", comments='')

							fig, ax = plt.subplots(1, 1, dpi = 300)
							ax.plot(time_solution, N_solution, 'b-', label=r'$\mathcal{N}$')
							ax.plot(time_solution, Q_solution, 'g-', label=r'$\mathcal{Q}$')
							ax.plot(time_solution, P_solution, 'r-', label=r'$\mathcal{P}$')
							ax.set(xlabel = r'Time (h)', ylabel = 'Number of cells')
